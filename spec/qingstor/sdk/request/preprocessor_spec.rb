...
module QingStor
  module SDK
    RSpec.describe Preprocessor do
      ...
      it 'processes qingstor request with endpoint' do
        config = Config.new.load_default_config.update(endpoint: 'ep.qingstor.dev')
        # Logger.set_level 'debug'
        input = {
          config:           config,
          properties:       {
        ...
        time_check = 'Thu, 01 Sep 2016 07:30:00 GMT'
        expect(result[:request_endpoint]).to eq 'http://beta.ep.qingstor.dev'
        expect(result[:request_headers][:'If-Modified-Since']).to eq time_check
        expect(result[:request_headers][:Range]).to eq '100-'
        expect(result[:request_uri]).to eq '/test/path/to/key.txt'
      end

      it 'processes qingstor request with vhost' do
        config = Config.new.load_default_config.update(endpoint: 'ep.qingstor.dev', enable_virtual_host_style: true)
        # Logger.set_level 'debug'
        input = {
          config:           config,
          properties:       {
        ...
        time_check = 'Thu, 01 Sep 2016 07:30:00 GMT'
        expect(result[:request_endpoint]).to eq 'http://test.beta.ep.qingstor.dev'
        expect(result[:request_headers][:'If-Modified-Since']).to eq time_check
        expect(result[:request_headers][:Range]).to eq '100-'
        expect(result[:request_uri]).to eq '/path/to/key.txt'
      end
    end
  end
end
